...
pub type OpEventLoopFn = dyn Fn(Rc<RefCell<OpState>>, &mut Context, v8::Global<v8::Context>) -> bool;
...
impl Extension {
  ...
  pub fn run_event_loop_middleware(
    ...
    cx: &mut Context,
    isolate: v8::Global<v8::Context>
  ) -> bool {
    self
      .event_loop_middleware
      .as_ref()
      .map(|f| f(op_state_rc, cx, isolate))
      .unwrap_or(false)
  }
...
impl ExtensionBuilder {
  ...
  where
    F: Fn(Rc<RefCell<OpState>>, &mut Context, v8::Global<v8::Context>) -> bool + 'static,
  {
