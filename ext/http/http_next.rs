...
pub fn op_serve_http<'scope>(
  ...
  let init_callback: v8::Local<v8::Function> =
    v8::Local::<v8::Function>::try_from(init_cb.v8_value)?;
  let recv = v8::undefined(scope);
  let scheme = v8::String::new(scope, listen_properties.scheme).unwrap();
  let fallback_host =
    v8::String::new(scope, listen_properties.fallback_host.as_str()).unwrap();
  init_callback.call(
    scope,
    recv.into(),
    &[scheme.into(), fallback_host.into()],
  );
  ...
  let handle = spawn_local(async move {
    loop {
      let conn = listener
        ...
        .await?;
      serve_http_on(conn, &listen_properties, cancel_clone.clone(), tx.clone());
    }
  ...
  *RcRef::map(&resource, |this| &this.0)
  ...
  Ok(state.borrow_mut().resource_table.add_rc(resource))
}
